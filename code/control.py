...
combined_c_matrix_control, labels = generate_combined_matrix(func_directory_control, confounds_file, num_time_points, num_subjects)
...
for i, label1 in enumerate(labels):
    for j, label2 in enumerate(labels):
        if i < j and combined_c_matrix_control[i, j] > threshold:
            print(f"{label1} + {label2}: {combined_c_matrix_control[i, j]:.2f}")

...
def generate_combined_matrix(func_directory, confounds_file, num_time_points, num_subjects):
    ...
    func_files = sorted([f for f in os.listdir(
...
combined_c_matrix_control, labels = generate_combined_matrix(
    func_directory_control, confounds_file, num_time_points, num_subjects)


scaler = MinMaxScaler(feature_range=(0, 1))
combined_c_matrix_control = scaler.fit_transform(combined_c_matrix_control)


np.save('c_matrix_control.npy', combined_c_matrix_control)


fig_matrix, ax_matrix = plt.subplots(figsize=(12, 10))
cax_matrix = ax_matrix.matshow(combined_c_matrix_control, cmap='viridis')
...
for i, label1 in enumerate(labels):
    for j, label2 in enumerate(labels):
        if i < j and combined_c_matrix_control[i, j] > threshold:
            print(
                f"{label1} + {label2}: {combined_c_matrix_control[i, j]:.2f}")
